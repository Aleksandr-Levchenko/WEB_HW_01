...
from record_book import AddressBook, Record, Name, Phone, Email, Birthday, Address
from exceptions import PhoneException, BirthdayException, EmailException, FileOperation_Error
...
def main():    
    ...
    while True:
        ...
        if handler:
            ...
            print("[bold yellow]Pay attention[/bold yellow]: Command was not recognized!")
            continue
        
        save_address_book(path_book)
        save_notes_book(path_note)
...
def input_error(func):
    def inner(*prm):
        ...
        # Обробка виключних ситуацій
            ...
            print(e)
        except FileOperation_Error as e:
            print(e)   
...
def func_add_rec(args):
    if not str(args[0]).capitalize() in address_book.keys():
...
def add_phone(prm):
    ...
    if prm and (count_prm >= 2):
        if args[0].capitalize() in address_book.keys():
            rec = address_book[args[0].capitalize()]
            if address_book[args[0].capitalize()].phones[0].value == "None": 
                address_book[args[0].capitalize()].phones.clear()
...
def func_show_phonebook(_)->str:
    if len(address_book.data) == 0: 
        ...
        console = Console()
        _ = [table.add_row(str(record.name.value), str(', '.join(map(lambda phone: phone.value, record.phones))), str(record.email.value), str(record.birthday.value), str(record.address.value)) for record in address_book.data.values()]
        console.print(table)
        return ""
        
        # старий варіант друку таблиці
        # result = ""
        # result = "\n".join([f"{n}|{record.birthday.value}|{', '.join(map(lambda phone: phone.value, record.phones))}" for n, record in address_book.data.items()])
...
def func_book_pages(prm):
    ...
    n = int(re.sub("\D", "", prm))
    n_page = 0
    for batch in address_book._record_generator(N=n):
        n_page += 1
...
def func_phone(prm):
    ...
    if name in address_book.keys():   
        if prm: return ", ".join([phone.value for phone in address_book[name].phones])
...
def func_get_day_birthday(prm):
    ...
    if prm and (count_prm >= 1):
        if "/" in prm[0]:   # Example >> birthday /365
            ...
            return address_book.get_list_birthday(count_day)
            
        else: # Example >> birthday Mike
            name = prm[0].lower().capitalize()
            if name in address_book.keys():
                if address_book[name].birthday.value == "None": return f"No [bold red]Birthday[/bold red] for {name}"
                return address_book[name].days_to_birthday() 
...
@input_error
def remove_rec(prm:str):
    args = prm.split(" ")
    rec = address_book[args[1].capitalize()]
    if args[0].lower() == "name":
        if address_book[args[1].capitalize()].name.value == args[1].capitalize():
            del address_book[args[1].capitalize()]
...
@input_error
def change_rec(prm:str):
    args = prm.split(" ")
    if args[1].capitalize() in address_book.keys():
        rec = address_book[args[1].capitalize()]
        if args[0].lower() == "name":
            if not args[2].capitalize() is address_book.data.keys():
                rec = address_book[args[1].capitalize()]
                rec.change_name(Name(args[1].capitalize()), Name(args[2].capitalize()))
                address_book.data.pop(args[1].capitalize())
                address_book[args[2].capitalize()] = rec
...
#=========================================================
# Функція читає базу address_book з файлу *.bin
#========================================================= 
@input_error
def load_address_book(path):
    return address_book.load_database(path)

#=========================================================
# Функція виконує збереження address_book у файл *.bin
#========================================================= 
@input_error
def save_address_book(path):
    return address_book.save_database(path)

#=========================================================
# Функція читає базу notes_book з файлу *.json
#========================================================= 
@input_error
def load_notes_book(path):
    return note_book.load_database(path)

#=========================================================
# Функція виконує збереження notes_book у файл *.json
#========================================================= 
@input_error
def save_notes_book(path):
    return note_book.save_database(path)
    
    
#=========================================================
# Функція виконує парсер команд та відповідних параметрів
#=========================================================
def parcer_commands(*args):
    if args:
        for handler, cmd in COMMANDS.items():
            if args[0] in cmd:
                prm = args[1:]
                return handler, prm
        return None, []
...
COMMANDS = {clear_screen:       ("cls",),
            sort_dir:           ("sort_dir",),
            func_help:          ("help",),
            func_exit:          ("good_buy", "exit", "close"),
            func_greeting:      ("hello",),
            load_address_book:  ("load_book",),
            save_address_book:  ("save_book",),
            func_add_rec:       ("add_rec",),
            func_show_phonebook:("show_book",),
            add_phone:          ("add_phone",),
            add_email:          ("add_email",),
            add_address:        ("add_address",),
            add_birthday:       ("add_birthday",),
            remove_rec:         ("rem_rec"),
            change_rec:         ("change_rec", "chg_note"),
            search_rec:         ("search_rec", "sch_note"),
            note_add:           ("add_note",),
            note_del:           ("del_note"),
            note_change:        ("change_note", "chg_note"),
            note_find:          ("find_note",),
            note_show:          ("show_notes",),
            note_sort:          ("sort_note",),
            func_phone:         ("phone",),
